...
### 1. Generalised Boosted Model

First model is GBM which is being trained with repeated cross validation resampling using 3 folds with one repeat.
...
accuracy_gbm
```
We have receive an Accuracy of `r accuracy_gbm` for  Generalised Boosted Model


### 2. Randon Forest

Second model being used is Random Forest.
```{r Randam Forest Model Fit}
...
accuracy_rf
```
We receive Accuracy of `r accuracy_rf` for Random Forest, which is higher than that of GBM and thus we will use Random Forest for prediction on test set.
