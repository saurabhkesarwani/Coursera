...
First model being trained is GBM with repeated cross validation resampling using 3 folds.
...
accuracy_gbm
```
We have receive an Accuracy of `r (round(accuracy_gbm, digits = 4)*100)` for  Generalised Boosted Model
...
accuracy_rf
```
We receive Accuracy of `r (round(accuracy_rf, digits = 4)*100)` for Random Forest, which is higher than that of GBM and thus we will use Random Forest for prediction on test set.
